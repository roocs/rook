...
- **F. S3 storage:** initial support and validation still needed
...
# F1. S3 storage support

## Work in progress

- Initial support accepts **`s3://` inputs** for processing data in object storage.
- The input layer handles **NetCDF files, Zarr stores and Kerchunk references**.
- Shared **S3 configuration** provides the endpoint and anonymous or authenticated access.
- Catalog datasets can use a **global S3 base path** or a **per-project override**.

**Not finished yet: needs end-to-end testing with real S3 storage.**
